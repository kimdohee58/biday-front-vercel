...
import PrivacyPolicyContent from "./PrivacyPolicyModal";
...
const TermsAgreement = () => {
    ...
    const [privacyOptionChecked, setPrivacyOptionChecked] = useState(false);
    const [adsChecked, setAdsChecked] = useState(false);
    const [showTermsModal, setShowTermsModal] = useState(false);
    const [showPrivacyModal, setShowPrivacyModal] = useState(false);
    const [showPrivacyOptionModal, setShowPrivacyOptionModal] = useState(false);
    const [showAdsOptions, setShowAdsOptions] = useState(false);

    const [emailChecked, setEmailChecked] = useState(false);
    const [appPushChecked, setAppPushChecked] = useState(false);
    ...
    const handleAllChecked = () => {
        ...
        setPrivacyOptionChecked(newState);
        setAdsChecked(newState);
        setEmailChecked(newState);
        setAppPushChecked(newState);
        setSmsChecked(newState);
    };

    const handleAdsChecked = () => {
        const newState = !adsChecked;
        setAdsChecked(newState);
        setEmailChecked(newState);
        setAppPushChecked(newState);
        setSmsChecked(newState);
        ...
        <div className="terms-agreement-section">
            ...
            <div className="flex items-center mt-2 justify-between">
                <div className="flex items-center">
                    <input
                        type="checkbox"
                        checked={privacyOptionChecked}
                        onChange={() => setPrivacyOptionChecked(!privacyOptionChecked)}
                        className="mr-2"
                    />
                    <span>[선택] 개인정보 수집 및 이용 동의</span>
                </div>
                <button
                    onClick={() => setShowPrivacyOptionModal(true)}
                    className="text-blue-500 hover:underline"
                >
                    내용 보기
                </button>
                                ...
                                <li>
                                    ...
                                    <ol className="list-disc list-inside mt-2">
                                        ...
                                        <li>사업자(이하 ‘사업자’에는 개인사업자 포함)가 이용신청을 하는 경우 (단, <span
                                            ...
                                            className="font-semibold">"제휴 사업자"</span>, <span className="font-semibold">"입점 사업자"</span> 또는 <span
            ...
            {showPrivacyOptionModal && (
                <Modal
                    ...
                    onClose={() => setShowPrivacyOptionModal(false)}
                />
            )}

            {
                showAdsOptions && ( // 펼치기 상태에 따라 표시
                    <div className="ml-4 mt-2">
                        <div className="flex items-center">
                            <input
                                type="checkbox"
                                checked={emailChecked} // 이메일 체크 상태
                                onChange={() => setEmailChecked(!emailChecked)} // 이메일 체크 변경 함수
                                className="mr-2"
                            />
                            <span>이메일 수신</span>
                        </div>
                        <div className="flex items-center mt-1">
                            <input
                                type="checkbox"
                                checked={appPushChecked} // 앱 푸시 체크 상태
                                onChange={() => setAppPushChecked(!appPushChecked)} // 앱 푸시 체크 변경 함수
                                className="mr-2"
                            />
                            <span>앱 푸시 수신</span>
                        </div>
                        <div className="flex items-center mt-1">
                            <input
                                type="checkbox"
                                checked={smsChecked} // 문자 메시지 체크 상태
                                onChange={() => setSmsChecked(!smsChecked)} // 문자 메시지 체크 변경 함수
                                className="mr-2"
                            />
                            <span>문자 메시지 수신</span>
                        </div>
